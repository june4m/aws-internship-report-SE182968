---
title: "Worklog Tuần 5"
date: "2025-09-08"
weight: 1
chapter: false
pre: " <b> 1.5. </b> "
---

{{% notice warning %}}
⚠️ **Lưu ý:** Các thông tin dưới đây chỉ nhằm mục đích tham khảo, vui lòng **không sao chép nguyên văn** cho bài báo cáo của bạn kể cả warning này.
{{% /notice %}}

### Mục tiêu tuần 5:

- Kết nối, làm quen với các thành viên trong First Cloud Journey.
- Hiểu dịch vụ AWS cơ bản, cách dùng console & CLI.

### Các công việc cần triển khai trong tuần này:

Tuần 5 – Quản trị Bảo mật Đám mây, Khám phá Serverless & Khởi động Dự án

| Ngày  | Nhiệm vụ                                                                                                                                                                                                                            | Ngày bắt đầu | Ngày hoàn thành | Tài liệu tham khảo          |
| :---- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------- | :-------------- | :-------------------------- |
| **2** | - Xem lý thuyết module 5: Các dịch vụ bảo mật trên AWS<br>- Mô hình trách nhiệm chia sẻ (Shared Responsibility)<br>- Amazon IAM<br>- AWS Cognito<br>- AWS Organizations<br>- AWS Identity Center<br>- AWS KMS<br>- AWS Security Hub | 06/10/2025   | 06/10/2025      | - IAM<br>- Cognito<br>- KMS |
| **3** | - Thảo luận và thống nhất về dự án nhóm<br>- Các dịch vụ sẽ sử dụng<br>- Dữ liệu (Data)<br>- Các tính năng                                                                                                                          | 07/10/2025   | 07/10/2025      | Biên bản cuộc họp           |
| **4** | - Tìm hiểu về các dịch vụ serverless (tính năng, giá cả, …)                                                                                                                                                                         | 08/10/2025   | 08/10/2025      | Dịch vụ (Service)           |
| **5** |                                                                                                                                                                                                                                     | 09/10/2025   | 09/10/2025      |                             |
| **6** |                                                                                                                                                                                                                                     | 10/10/2025   | 10/10/2025      |                             |

---

### Thu Hoạch Tuần 5: Quản Trị, Bảo Mật và Tư Duy Serverless

- **Quản lý Bảo mật & Danh tính AWS (AWS Security & Identity Management):** Đã củng cố kiến thức về Mô hình Trách nhiệm Chia sẻ (Shared Responsibility) và các công cụ quản lý danh tính (Cognito, Identity Center). Nắm vững vai trò của KMS và Security Hub trong việc đảm bảo trạng thái bảo mật tổng thể.
- **Nghiên cứu Kiến trúc Serverless (Serverless Architecture Research):** Đã hoàn thành việc nghiên cứu sâu về mô hình Serverless, tập trung vào Lambda, API Gateway, và DynamoDB. Đây là nền tảng cho việc triển khai dự án nhóm.
- **Khởi động Dự án Cuối khóa (Capstone Project Initiation):** Đã làm việc nhóm hiệu quả để thống nhất kiến trúc (sử dụng Serverless), phân chia nhiệm vụ, và thiết lập môi trường phát triển ban đầu. Dự án sẽ là cơ hội để áp dụng toàn bộ kiến thức đã học vào thực tế.
- **Quản trị (Governance):** Bắt đầu hiểu về AWS Organizations và SCPs (Service Control Policies) như một phương tiện để quản lý và kiểm soát tài nguyên trên nhiều tài khoản, chuẩn bị cho quy mô lớn hơn.
